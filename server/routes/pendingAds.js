...
router.get('/company/:companyId', authMiddleware, async (req, res) => {
  try {
    ...
    console.log('🔍 GET /company/:companyId - companyId:', companyId, 'status:', status);
    ...
    console.log('✅ Found', pendingAds.length, 'ads for company');

    res.json({ success: true, ads: pendingAds });

  } catch (error) {
    console.error('❌ Error in /company/:companyId:', error);
    res.status(500).json({
      success: false,
...
/* ---------------------------------------------
   GET - מודעה ספציפית (ציבורי - ללא אימות)
---------------------------------------------- */
...
router.post('/:id/approve', authMiddleware, async (req, res) => {
  try {
    ...
    if (rating) {
      pendingAd.companyFeedback = pendingAd.companyFeedback || {};
