...
export const AuthProvider = ({ children }) => {
  ...
  // ✅ Wrapped in useCallback to prevent dependency warnings
  const loadUserFromToken = useCallback(async () => {
    ...
    try {
      ...
      if (meData.success && meData.user) {
        ...
        // ✅ NEW: Fetch basic stats for company
  ...
  const handleLogin = async (email, password) => {
    try {
      ...
      // ✅ FIX: Set user immediately without additional fetch
      setUser(data.user);
      setIsInitialized(true);
      ...
      // ✅ Navigate immediately - let the dashboard load its own data
